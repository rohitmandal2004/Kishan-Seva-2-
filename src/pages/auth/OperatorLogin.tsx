...
export default function OperatorLogin() {
  ...
  // Auto-redirect if already logged in with a valid operator session
  useEffect(() => {
  ...
  const handleSendOtp = useCallback(async (e?: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Pre-auth verification: Operator profile must exist in Supabase
      if (isSupabaseConfigured()) {
        try {
          const { data: operatorProfile, error: dbError } = await supabase
          ...
          if (dbError) {
            console.error('[Kishan Seva] Operator check error:', dbError);
      ...
      // Start the sign-in process with Clerk using normalized email
      ...
      console.error('[Kishan Seva] Operator login OTP send error:', err);
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] text-blue-700 font-bold hover:underline disabled:text-slate-500 disabled:no-underline cursor-pointer"
                >
