...
import { 
  ...
  TrendingUp, TrendingDown, Sun, Cloud, Droplets, Bell, Building
} from 'lucide-react';
...
export default function FarmerDashboard() {
  ...
  // Derive precise farm coordinates from profile
  ...
  const notifications = store.getNotificationsForFarmer(farmer.id, user?.email).slice(0, 4);
  ...
  const estYieldPerAcre = 18; // approx 18 quintals per acre
  ...
  const openMarketRate = cropMsp.rate_per_quintal - (cropMsp.rate_per_quintal * 0.08); // 8% less in open market
    ...
    <AnimatedPage className="relative w-full h-full flex flex-col">
      ...
      <div className="p-4 md:p-6 max-w-6xl mx-auto w-full pb-24 md:pb-6 font-sans relative z-10 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="p-5 border border-slate-200/80 shadow-sm bg-white/90 backdrop-blur-sm rounded-2xl relative overflow-hidden group transition">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wider">Total Procured</span>
            ...
            <p className="text-[10px] text-slate-500 ml-[60px] font-medium">Historical sales</p>
          ...
          <Card className="p-5 border border-slate-200/80 shadow-sm bg-white/90 backdrop-blur-sm rounded-2xl relative overflow-hidden group transition">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <span className="text-[10px] font-medium text-slate-500 uppercase tracking-wider">DBT Disbursed</span>
          ...
          {/* Yield Estimator */}
          <Card className="p-5 border border-slate-200/80 shadow-sm bg-gradient-to-br from-indigo-50/50 to-white backdrop-blur-sm rounded-2xl relative overflow-hidden group transition">
            <div className="flex justify-between items-start mb-2">
              <div>
                <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider flex items-center gap-1"><Sprout className="w-3 h-3" /> Yield Estimator</span>
                <p className="text-lg font-black text-slate-900 mt-1">{expectedTotalQuintals.toFixed(0)} Q <span className="text-xs font-semibold text-slate-500">est. harvest</span></p>
              </div>
          ...
          {/* Live Market Prices */}
          <Card className="p-5 border border-slate-200/80 shadow-sm bg-gradient-to-br from-emerald-50/50 to-white backdrop-blur-sm rounded-2xl relative overflow-hidden group transition">
            <div className="flex justify-between items-start mb-2">
              <div>
                <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider flex items-center gap-1"><TrendingUp className="w-3 h-3" /> Rate Advantage</span>
                <p className="text-lg font-black text-emerald-700 mt-1">₹{cropMsp.rate_per_quintal} <span className="text-xs font-semibold text-slate-500">/ Q (MSP)</span></p>
              </div>
        ...
        {activeBooking ? (
          <Card className="p-4 sm:p-5 border border-emerald-200/60 bg-white/90 backdrop-blur-md shadow-sm rounded-2xl relative overflow-hidden group">
            ...
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 mb-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button variant="outline" className="border-slate-300 text-slate-700 rounded-lg text-xs font-bold px-4 h-10 gap-2 bg-white hover:bg-slate-50 shadow-sm transition-colors">
                  <Calendar className="w-4 h-4 text-slate-400" /> View Details
                </Button>
              </div>
            </div>
          ...
          bestCentreRec && (
            <Card className="p-4 sm:p-5 border border-slate-200/80 bg-white/90 backdrop-blur-md shadow-sm rounded-2xl relative overflow-hidden group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="md:col-span-2 p-4 border border-slate-200 shadow-sm bg-white rounded-2xl flex items-center justify-between">
            <h3 className="font-extrabold text-slate-900 text-sm hidden sm:block shrink-0 mr-4">Quick Links</h3>
            <div className="flex gap-2 w-full justify-between sm:justify-end">
              <Link to="/farmer/book" className="flex flex-col items-center gap-1.5 group">
                <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-emerald-700 group-hover:bg-emerald-50 transition-colors">
              ...
              <Link to="/farmer/centres" className="flex flex-col items-center gap-1.5 group">
                <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-amber-600 group-hover:bg-amber-50 transition-colors">
          ...
          <Card className="md:col-span-1 p-4 border border-emerald-100 shadow-sm bg-emerald-50/50 rounded-2xl flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="p-5 border border-slate-200/80 shadow-sm bg-white rounded-2xl transition">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2"><Sun className="w-4 h-4 text-amber-500" /> Harvest Weather Advisory</h3>
              <span className="text-[10px] text-slate-500 font-medium">{farmer.village}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {weatherForecast.map((w, idx) => (
                <div key={idx} className={`p-2.5 rounded-xl border ${w.isGoodForHarvest ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'} text-center flex flex-col items-center justify-between h-full`}>
                  ...
                  <p className="text-[11px] font-black text-slate-800">{w.temp}°C</p>
          ...
          <Card className="p-5 border border-slate-200/80 shadow-sm bg-white rounded-2xl flex flex-col transition">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2"><Bell className="w-4 h-4 text-blue-500" /> Recent Updates</h3>
          ...
          <div className="space-y-4">
            {allBookings.length === 0 ? (
              ...
              allBookings.map((b) => {
                ...
                return (
                  <div key={b.id} className="p-4 rounded-xl border border-slate-200 bg-white hover:border-emerald-300 shadow-xs transition flex flex-col gap-4">
                    {/* Header Row */}
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
                        ...
                        {isCompleted ? (
                          ...
                          <Link to="/farmer/queue">
                            <Button size="sm" className="bg-emerald-700 hover:bg-emerald-800 text-white text-[11px] h-8 rounded-lg px-4">Track</Button>
                          </Link>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="mt-2 pt-3 border-t border-slate-100 bg-slate-50/50 -mx-2 -mb-2 px-2 pb-2 rounded-b-lg">
                        <div className="flex items-center justify-between">
                          <div className="flex-1 flex items-center gap-2 relative">
                            ...
                            <div className={`absolute top-1/2 left-4 h-0.5 bg-emerald-500 -z-10 -translate-y-1/2 transition-all duration-500`} style={{ width: stage === 3 ? 'calc(100% - 32px)' : stage === 2 ? '50%' : '0%' }}></div>
                            
                            {/* Stage 1: Initiated */}
                            <div className="flex flex-col items-center gap-1 flex-1">
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] ${stage >= 1 ? 'bg-emerald-500 text-white shadow-md' : 'bg-white border-2 border-slate-300 text-slate-400'}`}>
                                <CheckCircle2 className="w-3 h-3" />
                              </div>
                              <span className={`text-[9px] font-bold ${stage >= 1 ? 'text-emerald-700' : 'text-slate-400'}`}>Initiated</span>
                            </div>

                            {/* Stage 2: Treasury */}
                            <div className="flex flex-col items-center gap-1 flex-1">
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] ${stage >= 2 ? 'bg-emerald-500 text-white shadow-md' : 'bg-white border-2 border-slate-300 text-slate-400'} ${stage === 2 && 'ring-2 ring-emerald-200 ring-offset-1 animate-pulse'}`}>
                                <Building className="w-3 h-3" />
                              </div>
                              <span className={`text-[9px] font-bold ${stage >= 2 ? 'text-emerald-700' : 'text-slate-400'}`}>Treasury Process</span>
                            </div>

                            {/* Stage 3: Credited */}
                            <div className="flex flex-col items-center gap-1 flex-1">
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] ${stage === 3 ? 'bg-emerald-500 text-white shadow-md' : 'bg-white border-2 border-slate-300 text-slate-400'}`}>
                                <Banknote className="w-3 h-3" />
                              </div>
                              <span className={`text-[9px] font-bold ${stage === 3 ? 'text-emerald-700' : 'text-slate-400'}`}>Credited</span>
                            </div>
                          </div>
                          
                          <div className="w-1/3 text-right">
                            <span className="text-[10px] font-semibold text-slate-500 block mb-0.5">DBT Value</span>
                            <span className="text-sm font-black text-slate-900">₹{b.weighment_data.net_payable.toLocaleString('en-IN')}</span>
                          </div>
                        </div>
                      </div>
                    )}
        ...
        {/* Advisory & Guidelines */}
        <div className="p-3 rounded-xl bg-amber-50/80 border border-amber-200/80 flex items-start gap-2.5 text-[11px] text-amber-900 mb-6">
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-5 sm:p-7 shadow-2xl relative animate-in fade-in zoom-in duration-200 max-h-[92vh] overflow-y-auto print-clean-card">
            ...
            <div className="pt-2 border-t border-slate-200 flex gap-3 print-hide">
              <Button 
                ...
                className="flex-1 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl text-xs font-bold h-10 gap-1.5 shadow-md transition"
              >
              ...
              <Button 
                onClick={() => setSelectedReceipt(null)}
                variant="outline" 
                className="rounded-xl text-xs font-bold h-10 px-5 transition"
              >
                Close
              </Button>
            </div>
