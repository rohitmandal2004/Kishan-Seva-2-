...
import { 
 ...
 Crosshair, Compass, SlidersHorizontal, LocateFixed
} from 'lucide-react';
...
export default function CentreDiscovery() {
 ...
 setCameraKey(`select-${coords.village}-${Date.now()}`);
 };
 ...
 setCameraKey(`centre-${item.centre.id}-${Date.now()}`);
 };
 ...
 sortBy === 'RECOMMENDED' ? 'bg-emerald-700 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
 }`}
 ...
 className={`px-2.5 py-1 rounded-full text-[10px] font-bold transition-colors ${
 sortBy === 'NEAREST' ? 'bg-emerald-700 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
 }`}
 ...
 <span className="font-extrabold text-emerald-950 underline">{activeVillage}</span>
 </span>
 </div>
 ...
 <span className="text-[10px] font-bold text-slate-400 uppercase shrink-0">Quick Select:</span>
 ...
 className={`px-2.5 py-0.5 rounded-full text-[11px] font-semibold transition shrink-0 border ${
 isCurrent 
 ...
 <span className="bg-red-100 text-red-800 text-[9px] font-extrabold px-1.5 py-0.5 rounded uppercase">
 Your Farm Origin
 </span>
 ...
 item.is_optimal ? 'bg-emerald-100 text-emerald-900' : item.is_nearest ? 'bg-amber-100 text-amber-900' : 'bg-slate-100 text-slate-700'
 }`}>
 ...
 className="bg-white/95 hover:bg-white text-slate-800 text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border border-slate-200 flex items-center gap-1.5 backdrop-blur-xs transition-transform active:scale-95"
 >
 ...
 {/* Map Legend Overlay */}
 <div className="absolute top-3 right-3 z-[400] bg-white/95 p-2 rounded-xl shadow-md border border-slate-200 text-[10px] font-medium space-y-1 backdrop-blur-xs hidden sm:block">
 <div className="flex items-center gap-1.5">
 ...
 <Card 
 key={item.centre.id} 
 className={`p-0 overflow-hidden cursor-pointer transition border-2 rounded-2xl ${
 ...
 <span className="text-xs font-black text-emerald-800 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full inline-block">
