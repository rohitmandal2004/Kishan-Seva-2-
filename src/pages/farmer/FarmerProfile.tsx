...
export default function FarmerProfile() {
 ...
 const handleUpdateVillage = async (newVillage: string) => {
 const coords = getCoordinatesForVillage(newVillage);
 const updated = {
 ...(farmer || {}),
 ...
 console.warn('Profile DB update error:', err);
 }
 }
 toast.success(`Address updated to ${coords.village} (${coords.district})`);
 };
 ...
 <p className="text-[10px] font-bold text-slate-400 uppercase">Registered Farm Address</p>
 {farmer?.latitude && (
 ...
 {farmer?.village || 'Basirhat'}, {farmer?.district || 'North 24 Parganas'}
 </p>
 <div className="mt-2 pt-2 border-t border-slate-100">
 <p className="text-[10px] font-semibold text-slate-500 mb-1.5">Quick Switch Farm Village:</p>
 <div className="flex flex-wrap gap-1.5">
 {['Basirhat', 'Diamond Harbour', 'Barasat', 'Habra', 'Baruipur', 'Canning'].map((vName) => {
 const isCurrent = (farmer?.village || '').toLowerCase() === vName.toLowerCase();
 return (
 <button
 key={vName}
 onClick={() => handleUpdateVillage(vName)}
