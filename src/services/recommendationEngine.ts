...
export async function getOsrmRoute(
 ...
 endLat: number,
 endLon: number
): Promise<{ distanceKm: number; durationMins: number; coordinates: [number, number][] } | null> {
 try {
...
export function evaluateCentreRecommendations(
 ...
 const hasCentreCoords =
 centre &&
 typeof centre.latitude === 'number' &&
 typeof centre.longitude === 'number';

 // Compute dynamic Haversine distance directly from the farmer's location
 const dist = hasFarmerCoords && hasCentreCoords
